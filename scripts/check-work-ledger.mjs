...
const demoMap = join(root, "docs/demos/rta-demo-coverage-map.md");
...
function hasListItemAfter(text, field) {
...
function scalarValue(text, field) {
...
function demoTargets(text) {
  return sectionLines(text, "demonstratedBy")
    .map((line) => line.trim())
    .flatMap((line) => {
...
for (const file of files) {
  ...
  if (idMatch) {
    ...
    if (!demoText.includes(idMatch[1]) && !demoText.includes(scalarValue(text, "name") ?? "")) {
      errors.push(`${rel} is not mentioned in the demo coverage map by id or name`);
    }
  }

  for (const demo of demoTargets(text)) {
    if (demo.type === "doc" && !existsSync(join(root, demo.target))) {
