...
const generateRepository = (
  ...
  return {
    ...
    content: [
      ...
      `  ${name}Repository,`,
      `  {`,
      `    readonly findById: (id: ${idName}) => Effect.Effect<${name}, NotFound>`,
      `    readonly save: (aggregate: ${name}) => Effect.Effect<void>`,
      `    readonly nextId: () => Effect.Effect<${idName}>`,
      `  }`,
      `>() {}`,
      ``,
      `// ---------------------------------------------------------------------------`,
      ...
      `let _${name.charAt(0).toLowerCase() + name.slice(1)}Counter = 0`,
      ``,
      `export const ${name}RepositoryInMemory = Layer.sync(${name}Repository, () => {`,
      `  return {`,
      `    findById: (id) =>`,
      `      _${name.charAt(0).toLowerCase() + name.slice(1)}Store.has(String(id))`,
      `        ? Effect.succeed(_${name.charAt(0).toLowerCase() + name.slice(1)}Store.get(String(id)) as ${name})`,
      `        : Effect.fail(new NotFound({ entityType: "${name}", id })),`,
      `    save: (agg) => Effect.sync(() => { _${name.charAt(0).toLowerCase() + name.slice(1)}Store.set(String(agg.id), agg) }),`,
      `    nextId: () => Effect.sync(() => \`${agg.name.toLowerCase()}-\${++_${name.charAt(0).toLowerCase() + name.slice(1)}Counter}\` as ${idName}),`,
      `  }`,
      `})`,
      ``,
    ].join("\n"),
...
const generateCommandHandler = (
  ...
  return {
    ...
    content: [
      ...
      ``,
      ...(strict ? [
        `export class ${className} extends InstrumentedCommandHandler<${cmdType}, NotFound, ${repoService}> {`,
        `  constructor() {`,
        ...
        `  ): Effect.Effect<void, NotFound, ${repoService}> {`,
