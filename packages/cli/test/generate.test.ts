...
describe("generateContext — @rta/strict mode", () => {
  ...
  it("generates a strict CQRS and event dispatch surface in the registry", () => {
    ...
    expect(registry).toContain(`const messageContext = {`)
    expect(registry).toContain(`correlationId: dispatchScope.traceId`)
    expect(registry).toContain(`causationId: dispatchScope.operationId`)
    expect(registry).toContain(`issuedAt: dispatchScope.clock.now()`)
