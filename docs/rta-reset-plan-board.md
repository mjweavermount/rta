...
| RTA-RESET-05 | Demo Ready | Rebuild vocab, tiers, patterns, and archetypes | Vocab can declare primitives, app-local concrete extensions, reusable patterns, and archetypes with transitive inherited obligations, concrete fields, and operation event requirements. | Package-local TypeScript tests and production checks prove tier blooming, circular bloom rejection, and inherited operation events. |
...
| RTA-RESET-13 | Demo Ready | Purge active JS/MJS and move to Effect-first TypeScript | RTA source is tracked TypeScript only. Effect services/layers still need deeper runtime IO, repositories, edge boundaries, secrets, review, queue, logs, and generated app execution work. | `pnpm check:pure-ts` passes with an empty allowlist and `pnpm check` includes the gate. |
...
| RTA-RESET-15 | Demo Ready | Add governed tool surfaces and runtime capabilities | MCP/HTTP/GraphQL-style tool surfaces and runtime capability adapters are core vocab/pattern/archetype concepts without hard-coding app leaves like a production Vault path. | `@rta/vocab` parses and rejects governed tool-surface declarations; golden fixtures include tool-surface, credential-broker, runtime-capability-adapter, projection-mount patterns, and an `mcp-gateway` archetype; `pnpm check:production` validates the new contracts. |
